    ...
    const popContent = (
        <>
            <div style={{maxWidth: "250px"}}>
                {showTags.length != 0 ? showTags.map((tag) => (<Tag key={tag}>{tag}</Tag>)) :
                    <Tag>Keine Filter ausgewählt</Tag>}
            </div>
    ...
    return (
        <Space.Compact size="large" direction="vertical" style={{margin: "auto"}}>
            <Space.Compact size="large" style={{margin: "auto"}}>
                <Popover placement="bottomLeft" title={"Nach Kategorie Filtern"} arrow={false} trigger="click"
                         content={popContent}>
                    <Button>
