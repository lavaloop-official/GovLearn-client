...
function Feedback({ review }: { review: Review }) {
    ...
    const handleOk = () => {
        fetchWrapper.post(`api/v1/reports/feedback/${review.feedbackID}`, { report_message: reportMessage, report_reason: (value - 1) }).then(res => {
    ...
    const content = (
        <>
            ...
            <Modal title="Bewertung melden" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} centered footer={[
                ...
                <Radio.Group onChange={onChange} value={value}>
                    <Space direction="vertical">
                        <Radio value={1}>Unangemessen</Radio>
                        <Radio value={2}>Kein Bezug</Radio>
                        <Radio value={3}>Sonstiges</Radio>
                    </Space>
                </Radio.Group>
                <br />
                <br />
                <TextArea rows={4} placeholder="Warum verstößt diese Bewertung gegen unsere Richtlinien?" onChange={onChangeReportMessage} />
            </Modal>
        </>
